...
import { ListToolsInterface } from '@inworld/runtime/common';
import {
  ComponentFactory,
  GraphBuilder,
  NodeFactory,
...
const createExecutor = (serverType: string, port: number) => {
  ...
  return new GraphBuilder('node_mcp_list_tools_graph')
...
async function run() {
  ...
  if (result.data) {
    if (result.type === 'LIST_TOOLS_DATA') {
      const listToolsData = result.data as ListToolsInterface;
      console.log(`\nAvailable tools from ${serverType} server:`);
      for (const tool of listToolsData.list_tools) {
        console.log('✓ Tool name:', tool.tool.name);
        console.log('  Description:', tool.tool.description);
        console.log('  Properties:', tool.tool.properties);
        console.log('');
      }
    } else {
      throw new Error(`Result is not LIST_TOOLS_DATA: ${result.data}`);
    }
  } else {
    throw new Error(`No data received from LLM_CHAT_REQUEST`);
  }
