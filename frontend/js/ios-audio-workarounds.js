...
class IOSAudioHandler {
    ...
    document.addEventListener('visibilitychange', async () => {
      if (!document.hidden && this.audioContext) {
        if (this.audioContext.state === 'suspended') {
          ...
          console.log(
            '[iOS Audio] Resumed audio context after visibility change'
          );
        }
      }
    });
  ...
  async unlockAudioContext() {
    try {
      ...
      console.log(
        '[iOS Audio] AudioContext unlocked, state:',
        this.audioContext.state
      );

      // Dispatch custom event to notify app
      window.dispatchEvent(
        new CustomEvent('ios-audio-unlocked', {
  ...
  async startMicrophone(onAudioData) {
    try {
      ...
      if (error.name === 'NotAllowedError') {
        alert(
          'Microphone access denied. Please enable microphone permissions in Settings > Safari > Microphone.'
        );
  ...
  async playAccumulatedAudio() {
    ...
    try {
      console.log(
        '[iOS Audio] Processing accumulated audio chunks:',
      ...
      const audioElement =
        document.getElementById('iosAudioElement') ||
        ...
        playPromise
          ...
          .catch(async (error) => {
            ...
            if (error.name === 'NotAllowedError') {
              ...
              console.log(
                '[iOS Audio] Waiting for user interaction to play audio...'
              );
              ...
              const playOnInteraction = async () => {
                try {
                  ...
                  console.log(
                    '[iOS Audio] Playback started after user interaction'
                  );
  ...
  getOptimizedWebSocketURL() {
    ...
    if (this.isIOS && hostname === 'localhost') {
      ...
      console.warn(
        '[iOS Audio] Using localhost, consider using computer IP address for iOS testing'
      );
    }
  ...
  handleIOSError(error, context) {
    ...
    const errorMessages = {
      NotAllowedError:
        'Permission denied. Please allow microphone/audio access.',
    ...
    const message =
      errorMessages[error.name] || `Audio error: ${error.message}`;

    // Dispatch error event for app to handle
    window.dispatchEvent(
      new CustomEvent('ios-audio-error', {
