...
export class AudioBuffer extends EventEmitter {
  ...
  extractSegment(
    ...
    const samplesToGoBack = Math.floor(
      (now - startTimestamp) * this.sampleRate
    );
    const startIndex =
      (this.writeIndex - samplesToGoBack + this.bufferSize) % this.bufferSize;
  ...
  addEvent(type: AudioEvent['type'], data?: unknown): void {
