...
// VAD type from the runtime library - using unknown since the exact type isn't exported
type VADInstance = {
...
export class SileroVAD extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      console.log(
        'SileroVAD: Starting initialization with model path:',
      ...
      console.log(
        'SileroVAD: Available devices:',
        availableDevices.map((d) => d.getType())
      );
  ...
  private async processAudioChunk(chunk: AudioChunk): Promise<void> {
    ...
    const totalSamples = this.accumulatedSamples.reduce(
    ...
    if (totalSamples >= this.FRAME_PER_BUFFER) {
      ...
      try {
        ...
        const hasVoiceActivity =
          result !== -1 && rmsVolume >= this.config.minVolume;
  ...
  private async processVADResult(
    ...
    if (this.isCapturingSpeech) {
      ...
      if (!hasVoiceActivity) {
        ...
        this.pauseDuration +=
          (audioChunk.length * 1000) / this.INPUT_SAMPLE_RATE; // ms

        if (this.pauseDuration > this.config.minSilenceDuration * 1000) {
          // Convert to ms
          this.isCapturingSpeech = false;

          const speechDuration =
            (this.speechBuffer.length * 1000) / this.INPUT_SAMPLE_RATE; // ms
  ...
  private async processCapturedSpeech(): Promise<void> {
    ...
    const speechDuration =
      (this.speechBuffer.length * 1000) / this.INPUT_SAMPLE_RATE;
