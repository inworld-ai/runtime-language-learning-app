...
import { float32ToPCM16 } from '../../audio/audio_utils.js';
...
class AssemblyAISession {
  ...
  // Audio buffering for AssemblyAI's chunk size requirements (50-1000ms)
  private audioBuffer: Int16Array = new Int16Array(0);
  // Buffer to ~100ms worth of audio before sending (1600 samples at 16kHz)
  private readonly MIN_SAMPLES_TO_SEND = 1600;
  ...
  public sendAudio(pcm16Data: Int16Array): void {
    ...
    const newBuffer = new Int16Array(this.audioBuffer.length + pcm16Data.length);
    newBuffer.set(this.audioBuffer, 0);
    newBuffer.set(pcm16Data, this.audioBuffer.length);
    this.audioBuffer = newBuffer;

    // Send if we have enough samples (100ms worth at 16kHz = 1600 samples)
    if (this.audioBuffer.length >= this.MIN_SAMPLES_TO_SEND) {
      this.ws.send(Buffer.from(this.audioBuffer.buffer));
  ...
  /**
   * Flush any remaining buffered audio (call before closing)
   */
  public flushAudio(): void {
    if (this.ws && this.ws.readyState === WebSocket.OPEN && this.audioBuffer.length > 0) {
      ...
      if (this.audioBuffer.length < 800) {
        // Pad to at least 50ms (800 samples)
        const paddedBuffer = new Int16Array(800);
        paddedBuffer.set(this.audioBuffer, 0);
        this.ws.send(Buffer.from(paddedBuffer.buffer));
      } else {
        this.ws.send(Buffer.from(this.audioBuffer.buffer));
      }
      this.audioBuffer = new Int16Array(0);
...
export class AssemblyAISTTWebSocketNode extends CustomNode {
  ...
  constructor(props: { id?: string; config: AssemblyAISTTWebSocketNodeConfig }) {
    ...
    console.log(
      `[AssemblyAI] Configured with [threshold:${this.endOfTurnConfidenceThreshold}] [silence:${this.minEndOfTurnSilenceWhenConfident}ms] [lang:${this.defaultLanguage}]`
    );
  }
  ...
  private buildWebSocketUrl(sessionId?: string): string {
    ...
    console.log(
      `[AssemblyAI] Connecting with [lang:${language}] [threshold:${endOfTurnThreshold}]`
    );
